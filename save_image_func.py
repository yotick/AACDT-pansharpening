...
def save_image_RS(sate, file_name, fused):
    ...
    if sate == 'wv3_8':
        save_path_fused = join(r'fused/WorldView-3-8/wv3-8b', file_name)    ## need to change here
        save_path_rgb = join('fused/WorldView-3-8/wv3-8-rgb', ''.join([file_name[:-4], '.tif'])) ## need to change here
    ...
    if sate == 'ik':
        save_path_fused = join(r'fused/IKONOS_AIFE/ik_4b', file_name)
        save_path_rgb = join('fused/IKONOS_AIFE/ik_rgb', ''.join([file_name[:-4], '.tif']))
...
def save_image_full(sate, file_name, fused):
    ...
    if sate == 'wv3_8':
        save_path_fused = join(r'fused/WorldView-3-8_loss/wv3-8b-FS', file_name)
        save_path_rgb = join('fused/WorldView-3-8_loss/wv3-8-rgb-FS', ''.join([file_name[:-4], '.tif']))
        # save_path_ms_up = join(r'../fused\WorldView-3-8\wv3-8-up', file_name)
