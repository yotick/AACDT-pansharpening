...
# file_names = ['wv3_0_TF.pt', 'wv3_0_WF.pt']
# file_name = 'good_35_res.pt'     #### need to change here ####################
file_path = 'visualize\\'
file_names = [file_name for file_name in os.listdir(file_path) if file_name.endswith('.pt')]
# tensor = torch.load('output_0_swin.pt').cpu()

for i in range(2):
    # 选择第一个通道的图像
    channel = 5*i+1             #### need to change here ####################
    for file_name in file_names:
        tensor = torch.load(file_path + file_name).cpu()
        ...
        plt.savefig(file_path + save_name, bbox_inches='tight')
