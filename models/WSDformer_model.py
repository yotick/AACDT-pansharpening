...
class WSDA(nn.Module):   ### 论文中的 Fig. 2. The structure of Wavelet-Spatial Dual Attention
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        if self.w_heads > 0:
            ...
            self.filter = nn.Sequential(
                nn.Conv2d(self.w_dim, self.w_dim, kernel_size=3, padding=1, stride=1, groups=1), ## chagne groups from 1
                # build_norm_layer(dict(type='BN', requires_grad=False), dim)[1],
                nn.BatchNorm2d(self.w_dim, affine=True),
                # nn.Conv2d(self.w_dim, self.w_dim, kernel_size=3, padding=1, stride=1, groups=self.w_dim), ## added
                nn.ReLU(inplace=True),
            )
            ...
            # self.w_kv_embed = nn.Conv2d(self.w_dim-2, self.w_dim,
            #                             kernel_size=sr_ratio, stride=sr_ratio) if sr_ratio > 1 else nn.Identity()
            self.w_kv_embed = nn.Conv2d(self.w_dim - 2, self.w_dim, kernel_size=3, stride=1)
            # self.w_q = nn.Linear(self.dim, self.w_dim, bias=qkv_bias)
            self.w_q = nn.Linear(2, self.w_dim, bias=qkv_bias)
            self.w_kv = nn.Sequential(
                nn.LayerNorm(self.w_dim),
                nn.Linear(self.w_dim, self.w_dim * 2)
            )
            self.w_proj = nn.Linear(self.w_dim // 2, self.dim//2)
    ...
    def wa2(self, x):
        ...
        #### for visualize ###########################################
        # b,n,c = wa_out.shape
        # sa_out_vis = sa_out.permute(0, 3, 1, 2)
        # wa_out_vis = wa_out.permute(0, 3, 1, 2)
        # file_path = 'visualize\\'
        # feature_save(x, file_path, 'x')  ##### save  visualization results
        # feature_save(y, file_path, 'y')  ##### save  visualization results

        x_idwt = self.idwt(x_dwt)
        # feature_save(x_idwt, file_path, 'x_idwt')  ##### save  visualization results
        x_idwt = x_idwt.view(B, -1, x_dwt.size(-2), x_dwt.size(-1) )
        # x_idwt = x_idwt.view(B, -1, x_idwt.size(-2) * x_idwt.size(-1)//4).transpose(1, 2)

        ca = x_dwt[:,0:2, :, :]
        cd = x_dwt[:,2:, :, :]
        q = self.w_q(ca.permute(0, 2, 3, 1)).reshape(B, H * W//4, self.w_heads, self.w_dim // self.w_heads).permute(0, 2, 1, 3)
        kv1 = self.w_kv_embed(cd)
        kv = kv1.reshape(B, self.w_dim, -1).permute(0, 2, 1)
        # kv = x_idwt
        ...
        attn = attn.softmax(dim=-1)
        x_att = (attn @ v).transpose(1, 2).reshape(B, H*W//4, self.w_dim)
        x_out = self.w_proj(torch.cat([x_idwt.reshape(B, H*W, self.w_dim//4), x_att.reshape(B, H*W, self.w_dim//4)], dim=-1))
        x_out = x_out.reshape(B, H, W, self.dim//2)
        ...
        # wa_out_vis = wa_out.permute(0, 3, 1, 2)
        # file_path = 'visualize\\'
        # feature_save(x_att, file_path, 'ca')  ##### save  visualization results
        # feature_save(wa_out_vis, file_path, 'wa_out_vis')  ##### save  visualization results
    ...
    def forward(self, x, H, W):
        ...
        # wa_out_vis = wa_out.permute(0, 3, 1, 2)
        # file_path = 'visualize\\'
        # feature_save(sa_out_vis, file_path, 'sa_out_vis')  ##### save  visualization results
        # feature_save(wa_out_vis, file_path, 'wa_out_vis')  ##### save  visualization results
        # torch.save(sa_out_vis, file_path + 'sa_out_vis.pt')  ###　change here!! ############
        # torch.save(wa_out_vis, file_path + 'wa_out_vis.pt')  ###　change here!! ############
        ...
        x = x.reshape(B, N, C)
        # feature_save(x.reshape(B, C, H, W), file_path, 'sw_cat')  ##### save  visualization results
        return x
...
class Block(nn.Module):    ### WSDformer Block 论文中Fig.1 (b) 的内容 ####
    """ Transformer Block.
    ...
    def forward(self, x, H, W):
        ...
        #### for visualize ###########################################
        # sa_out_vis = sa_out.reshape(B, C//2, H, W)
        # wa_out_vis = wa_out.reshape(B, C//2, H, W)
        #
        # B,N,C = x.shape
        # file_path = 'visualize\\'
        # # feature_save(x.reshape(B,C,H,W), file_path, 'SW_integrate')  ##### save  visualization results
        # # feature_save(wa_out_vis, file_path, 'wa_out_vis')  ##### save  visualization results
        # x_block = x.transpose(1, 2).view(B, C, H, W)
        # feature_save(x_block, file_path, 'x_block')  ##### save  visualization results
        return x
...
class WSDLayer(nn.Module):    ## 这里是一个完整的块##
    def __init__(self,
                 dim,
                 depth,
                 num_heads,
    ...
    ############  visualize #####################
        # B,N,C = x.shape
        # file_path = 'visualize\\'
        # feature_save(x, file_path, 'SW_block')  ##### save  visualization results
        return x
...
def feature_save(tensor, path, name):
    ...
    cv2.imwrite(path + '/' + str(name) + '.png', inp)