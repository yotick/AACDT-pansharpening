...
parser.add_argument('--crop_size', default=80, type=int, help='training images crop size')  ### need to change
...
parser.add_argument('--batch_size', default=28, type=int, help='train epoch number')  ### need to change
...
parser.add_argument('-c', '--config', default='configs/config_HSIT.json', type=str,  # config_PANNET?
...
model = MainNet().to(device)  ### ｎｅｅｄ　 to change !!!!!
...
l1 = nn.L1Loss()
mse = nn.MSELoss()
spatial_loss = Spatial_Loss(num_chanel)
...
for epoch in range(start_epoch + 1, opt.num_epochs + 1):
    ...
    for ms_up_crop, ms_org_crop, pan_crop, gt_crop in train_bar:
        ...
        out = model(ms_up, ms_org, pan)  #### need to change!!!!!!!
        ...
        def sam_loss(fused, lr_ms):
            fused_norm = torch.norm(fused, dim=1, keepdim=True)
            lr_ms_norm = torch.norm(lr_ms, dim=1, keepdim=True)
            inner_product = torch.sum(fused * lr_ms, dim=1, keepdim=True)
            sam = torch.acos(inner_product / (fused_norm * lr_ms_norm + 1e-8))
            return torch.mean(sam)

        l1_loss = l1(out_image, target)
        spa_loss = spatial_loss(out_image, target)
        mse_loss = mse(out_image, target)
        sam_ls = sam_loss(out_image, target)
        # predicted_RGB = out_image[:,0:3, :,:]
        # target_RGB = target[:,0:3, :,:]

        # 选择要拼接的通道索引
        channels_to_concat = [1, 3, 5]
        predicted_RGB1 = torch.cat([out_image[:, idx, :, :].unsqueeze(1) for idx in channels_to_concat], dim=1)
        target_RGB1 = torch.cat([target[:, idx, :, :].unsqueeze(1) for idx in channels_to_concat], dim=1)
        VGG_loss1 = VGGPerceptualLoss(predicted_RGB1, target_RGB1, vggnet)

        # 选择要拼接的通道索引
        channels_to_concat2 = [2, 4, 6]
        predicted_RGB2 = torch.cat([out_image[:, idx, :, :].unsqueeze(1) for idx in channels_to_concat2], dim=1)
        target_RGB2 = torch.cat([target[:, idx, :, :].unsqueeze(1) for idx in channels_to_concat2], dim=1)
        VGG_loss2 = VGGPerceptualLoss(predicted_RGB2, target_RGB2, vggnet)
        ...
        #                                                                               out["x23"])
        total_loss = l1_loss + spa_loss + 0.08 * VGG_loss1+ 0.08 * VGG_loss2
        # total_loss = l1_loss + spa_loss + 0.1 * VGG_loss1
        # total_loss = l1_loss + spa_loss + 0.2 * sam_ls + 0.06 * VGG_loss1+ 0.06 * VGG_loss2
        # total_loss = l1_loss
        torch.autograd.backward(total_loss)

        if epoch % config["trainer"]["iter_size"] == 0 or epoch == len(train_loader) - 1:
            optimizerG.step()
    ...
    if epoch % val_step == 0:
        ...
        for ms_crop, pan_crop, gt_crop in val_bar:
            ...
            with torch.no_grad():  # validation
                ms_up = Variable(ms_up_crop)
                ms_org = Variable(ms_crop)
                pan = Variable(pan_crop)
                if torch.cuda.is_available():
                    ms_up = ms_up.float().cuda()
                    ms_org = ms_org.float().cuda()
                    pan = pan.float().cuda()
                # out = netG(z)  # 生成图片
                # out = model(pan_crop, ms_gray_crop)
                # out, out2, out2_d = model(z1, z2, z3, sate)

                out = overlap_crop_forward(ms_up, ms_org, pan, model, shave=4, min_size=16308,
                                           bic=None)  ##shave 是叠加 的部分
