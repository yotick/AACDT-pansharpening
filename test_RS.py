...
from models.multi_branch import MainNet
...
def test(test_data_loader, model, sate):
    ...
    for ms_up_crop, ms_org_crop, pan_crop, gt_crop in val_bar:
        ...
        with torch.no_grad():  # validation
            ...
            out = overlap_crop_forward(ms_up, ms, pan, model, shave=4, min_size=16308, bic=None)   ## 如果需要切割。
            end = time.time()
        output = out.cpu()
...
if __name__ == "__main__":
    ...
    if opt.satellite == 'ik':
        opt.checkpoint = 'ik_epoch_160.pth'
